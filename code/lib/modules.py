...
from lib.discrete_vae import DiscreteVAE
...
def train(dataloader, netG, netD, netC, text_encoder, image_encoder, optimizerG, optimizerD, scaler_G, scaler_D, args):
    ...
    # optimizerV = torch.optim.Adam(DiscreteVAE.parameters(), lr=args.lr_v, betas=(0.0, 0.9))
    ...
    for step, data in enumerate(dataloader, 0):
        ...
        with torch.cuda.amp.autocast() if args.mixed_precision else dummy_context_mgr() as mpc:
            ...
            fake, vae_loss = netG(noise, sent_emb)
            vae_weight = 0.1
        ...
        # whole D loss
        with torch.cuda.amp.autocast() if args.mixed_precision else dummy_context_mgr() as mpc:
            errD = errD_real + (errD_fake + errD_mis)/2.0 + errD_MAGP + vae_loss * vae_weight
        # update D
        if args.mixed_precision:
...
def sample(dataloader, netG, text_encoder, save_dir, device, multi_gpus, z_dim, stamp):
    ...
    for step, data in enumerate(dataloader, 0):
        ...
        with torch.no_grad():
            noise = torch.randn(batch_size, z_dim).to(device)
            fake_imgs,_ = netG(noise, sent_emb, eval=True)
...
def calculate_FID_CLIP_sim(dataloader, text_encoder, netG, CLIP, device, m1, s1, epoch, max_epoch, times, z_dim, batch_size):
    ...
    for time in range(times):
        ...
        #         print(f"First data index: {i * len(data[0])}")
        #     else:
        #         print("Data is empty or has no length")            
...
def generate_samples(noise, caption, model):
    with torch.no_grad():
        fake,_ = model(noise, caption, eval=True)
    return fake
